...
public class GameManagerTest
{
    ...
    public void LoadUnits_AddsUnitsFromContainers()
    {
        ...
        AssertThat(playerContainer.GetChildCount()).IsEqual(1);
        AssertThat(GetPrivateField<List<IUnitSystem>>(manager, "_playerUnits").Count).IsEqual(1);
        AssertThat(GetPrivateField<List<IUnitSystem>>(manager, "_enemyUnits").Count).IsEqual(1);
    }
    ...
    public void CheckWinLoseCondition_SetsVictory_WhenNoEnemiesAlive()
    {
        ...
        AssertThat(GetPrivateField<List<IUnitSystem>>(manager, "_playerUnits")[0].IsAlive).IsTrue();
        AssertThat(GetPrivateField<List<IUnitSystem>>(manager, "_enemyUnits")[0].IsAlive).IsFalse();
    ...
    public void CheckWinLoseCondition_SetsDefeat_WhenNoPlayersAlive()
    {
        ...
        AovDataStructures.GameOutcome outcome = GetPrivateField<AovDataStructures.GameOutcome>(manager, "_gameOutcome");
        AssertThat(GetPrivateField<List<IUnitSystem>>(manager, "_playerUnits")[0].IsAlive).IsFalse();
        AssertThat(GetPrivateField<List<IUnitSystem>>(manager, "_enemyUnits")[0].IsAlive).IsTrue();
    ...
    public void CheckUnitsLife_SetsAllUnitsDead()
    {
        ...
        List<IUnitSystem> playerUnits = GetPrivateField<List<IUnitSystem>>(manager, "_playerUnits");
        List<IUnitSystem> enemyUnits = GetPrivateField<List<IUnitSystem>>(manager, "_enemyUnits");

        List<IUnitSystem> units = playerUnits;
    ...
    public void CurrentTurnEnded_ReducesAllCooldowns()
    {
        ...
        SetPrivateField(manager, "_playerUnits", new List<IUnitSystem> { player });
        SetPrivateField(manager, "_enemyUnits", new List<IUnitSystem> { enemy });
