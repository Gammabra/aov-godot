...
public sealed partial class AovPlayer : CharacterBody3D, IInteractor
{
	...
	private void Initialize()
	{
		...
		if (!_tutorialManagerPath.IsEmpty)
		{
	...
	public override void _Input(InputEvent @event)
	{
		...
		if (_isTutorial && _tutorialManager != null && _tutorialManager.IsOnlyToggleInventory)
		{
		...
		if (_isTutorial && _tutorialManager != null && !_tutorialManager.CanMove)
			return;
		...
		if (_isTutorial && _tutorialManager != null && !_tutorialManager.CanToggleInventory)
			return;
		if (@event.IsActionPressed("open_inventory"))
			_explorationInventoryUI?.Toggle();
	}
