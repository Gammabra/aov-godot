...
public sealed class BladeDance : SkillSystem
{
	...
	public override void Use(UnitSystem caster, List<UnitSystem> targets, MapSystem? map)
	{
		...
		for (int i = 0; i < count; i++)
		{
			...
			Console.WriteLine($"{caster.UnitName}: {Name} hit {targets[i].UnitName}");
		}
	}
}
...
public sealed class PhantomStrike : SkillSystem
{
	...
	public override void Use(UnitSystem caster, List<UnitSystem> targets, MapSystem? map)
	{
		...
		Console.WriteLine($"{caster.UnitName}: {Name} penetrated armor for {rawDamage} on {targets[0].UnitName}");
	}
}
...
public sealed class ExecutionBlade : SkillSystem
{
	...
	public override void Use(UnitSystem caster, List<UnitSystem> targets, MapSystem? map)
	{
		...
		Console.WriteLine($"{caster.UnitName}: {Name} on {target.UnitName} (low HP: {isLowHp}, x{multiplier})");
	}
}
...
public sealed class CounterStance : SkillSystem
{
	...
	public override void Use(UnitSystem caster, List<UnitSystem> targets, MapSystem? map)
	{
		...
		Console.WriteLine($"{caster.UnitName}: {Name} – ATK buffed for {targets[0].UnitName}");
	}
}
...
public sealed class BurningSlash : SkillSystem
{
	...
	public override void Use(UnitSystem caster, List<UnitSystem> targets, MapSystem? map)
	{
		...
		Console.WriteLine($"{caster.UnitName}: {Name} burned {targets[0].UnitName}");
	}
}
